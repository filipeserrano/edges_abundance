...
```{r}
# plotting the predicted values for the best model
plot_increase1 = plot(ggeffects::ggpredict(increase_model, terms = c("reldist_leading_scaled", "Group"),
                                          typical = "weighted_mean", interval = "confidence"))+
  scale_fill_manual(values = cols1) +
  scale_color_manual(values = cols1) + theme_classic() +
  xlab("Relative distance to leading edge") +
  ylab("Probability of increasing")  
print(plot_increase1)

chosen_lat_values <- c(-1.5, 0, .5)
cols2 = c(paste0("lat_abs_scaled [", paste(chosen_lat_values[1], collapse = ","), "]") = "red",
          paste0("lat_abs_scaled [", paste(chosen_lat_values[2], collapse = ","), "]") = "green",
          paste0("lat_abs_scaled [", paste(chosen_lat_values[3], collapse = ","), "]") = "blue")


plot_increase2 = plot(ggeffects::ggpredict(increase_model, terms = c("reldist_leading_scaled", paste0("lat_abs_scaled [", paste(chosen_lat_values, collapse = ","), "]")),
                                          typical = "weighted_mean", interval = "confidence"))+
  scale_fill_manual(values = cols2) +
  scale_color_manual(values = cols2) + theme_classic() +
  xlab("Relative distance to leading edge") +
  ylab("Probability of increasing")  
...
cols3 = c("-1" = "orange",
          "0"= "purple",
          "1" = "black")
plot_increase2a = plot(ggeffects::ggpredict(increase_model, terms = c("lat_abs_scaled"),
                                         type = "fixed", typical = "mode", interval = "prediction"))+
  theme_classic() +
  xlab("Relative distance to leading edge") +
  ylab("Probability of increasing")  +
  scale_fill_manual(values = cols3) +
  scale_color_manual(values = cols3, labels = c("28.47º", "41.16º", "53.85º"))

print(plot_increase2a)
ggsave(plot = last_plot(),"predicted_increasing_leading.jpg")

# convert to absolute latitude: 
# (value* sd_used) + mean_used
...
``` {r}
global_model_constant = trends_populations %>% 
  dplyr::mutate(mindist_edge = pmin(reldist_trailing, reldist_leading),
                mindist_edge_scaled = scale(mindist_edge)) %>% 
                  glmmTMB(constant ~ -1 + mindist_edge_scaled + 
                                   (mindist_edge_scaled * Group) +                        
                                   (mindist_edge_scaled * lat_abs_scaled) +
                                   (lat_abs_scaled * Group) +
                                   (mindist_edge_scaled|Species:Group) +
                                   (1|last_year), 
                                 family = binomial,
                                 data = .,
                                 na.action = "na.fail")
...
summary(global_model_constant)

# dredge model selection - NOT WORKING!!!
dredge_selection_constant <- dredge(global.model = global_model_constant)
dredge_selection_constant


## NOT ELEGANT BUT DID IT ITERATIVELY
model_constant_null = trends_populations %>% 
  dplyr::mutate(mindist_edge = pmin(reldist_trailing, reldist_leading),
                mindist_edge_scaled = scale(mindist_edge)) %>% 
                  glmmTMB(constant ~ -1 +
                                   (mindist_edge_scaled|Species:Group) +
                                 ...
                                 data = .,
                                 na.action = "na.fail")
summary(model_constant_null)

model_constant0 = trends_populations %>% 
  dplyr::mutate(mindist_edge = pmin(reldist_trailing, reldist_leading),
                mindist_edge_scaled = scale(mindist_edge)) %>% 
                  glmmTMB(constant ~ -1 + mindist_edge_scaled  +
                                   (mindist_edge_scaled|Species:Group) +
                                 ...
                                 data = .,
                                 na.action = "na.fail")
summary(model_constant0)

model_constant1 = trends_populations %>% 
  dplyr::mutate(mindist_edge = pmin(reldist_trailing, reldist_leading),
                mindist_edge_scaled = scale(mindist_edge)) %>% 
                  glmmTMB(constant ~ -1 + Group +
                                   (mindist_edge_scaled|Species:Group) +
                                   (1|last_year), 
                                 family = binomial,
                                 data = .,
...
model_constant2= trends_populations %>% 
  dplyr::mutate(mindist_edge = pmin(reldist_trailing, reldist_leading),
                mindist_edge_scaled = scale(mindist_edge)) %>% 
                  glmmTMB(constant ~ -1 + lat_abs_scaled +
                                   (mindist_edge_scaled|Species:Group) +
                                   (1|last_year), 
                                 family = binomial,
                                 data = .,
...
summary(model_constant2)

model_constant3= trends_populations %>% 
  dplyr::mutate(mindist_edge = pmin(reldist_trailing, reldist_leading),
                mindist_edge_scaled = scale(mindist_edge)) %>% 
                  glmmTMB(constant ~ -1 + mindist_edge_scaled + Group +
                                   (mindist_edge_scaled|Species:Group) +
                                   (1|last_year), 
                                 family = binomial,
                                 data = .,
...
summary(model_constant3)

model_constant4= trends_populations %>% 
  dplyr::mutate(mindist_edge = pmin(reldist_trailing, reldist_leading),
                mindist_edge_scaled = scale(mindist_edge)) %>% 
                  glmmTMB(constant ~ -1 + mindist_edge_scaled + Group +
                                   (mindist_edge_scaled|Species:Group) +
...
model_constant4 = trends_populations %>% 
  dplyr::mutate(mindist_edge = pmin(reldist_trailing, reldist_leading),
                mindist_edge_scaled = scale(mindist_edge)) %>% 
                  glmmTMB(constant ~ -1 + mindist_edge_scaled  + lat_abs_scaled +
                                   (mindist_edge_scaled|Species:Group) +
                                   (1|last_year), 
                                 family = binomial,
                                 data = .,
                                 na.action = "na.fail")
summary(model_constant4)

model_constant5 = trends_populations %>% 
  dplyr::mutate(mindist_edge = pmin(reldist_trailing, reldist_leading),
                mindist_edge_scaled = scale(mindist_edge)) %>% 
                  glmmTMB(constant ~ -1 + Group  + lat_abs_scaled +
                                   (mindist_edge_scaled|Species:Group) +
                                   (1|last_year), 
                                 family = binomial,
                                 data = .,
                                 na.action = "na.fail")
summary(model_constant5)

model_constant6 = trends_populations %>% 
  dplyr::mutate(mindist_edge = pmin(reldist_trailing, reldist_leading),
                mindist_edge_scaled = scale(mindist_edge)) %>% 
                  glmmTMB(constant ~ -1 + mindist_edge_scaled  + lat_abs_scaled + Group +
                                   (mindist_edge_scaled|Species:Group) +
                                   (1|last_year), 
                                 family = binomial,
                                 data = .,
...
summary(model_constant6)

model_constant7 = trends_populations %>% 
  dplyr::mutate(mindist_edge = pmin(reldist_trailing, reldist_leading),
                mindist_edge_scaled = scale(mindist_edge)) %>% 
                  glmmTMB(constant ~ -1 + 
                                   (mindist_edge_scaled * Group) +
...
summary(model_constant7)

model_constant8 = trends_populations %>% 
  dplyr::mutate(mindist_edge = pmin(reldist_trailing, reldist_leading),
                mindist_edge_scaled = scale(mindist_edge)) %>% 
                  glmmTMB(constant ~ -1 + 
                                   (mindist_edge_scaled * lat_abs_scaled) +
...
summary(model_constant8)

model_constant9 = trends_populations %>% 
  dplyr::mutate(mindist_edge = pmin(reldist_trailing, reldist_leading),
                mindist_edge_scaled = scale(mindist_edge)) %>% 
                  glmmTMB(constant ~ -1 + 
                                   (Group * lat_abs_scaled) +
                                   (mindist_edge_scaled|Species:Group) +
                                   (1|last_year), 
                                 family = binomial,
                                 data = .,
...
summary(model_constant9)

model_constant10 = trends_populations %>% 
  dplyr::mutate(mindist_edge = pmin(reldist_trailing, reldist_leading),
                mindist_edge_scaled = scale(mindist_edge)) %>% 
                  glmmTMB(constant ~ -1 + lat_abs_scaled +
                                   (mindist_edge_scaled * Group) +
                                 ...
                                 data = .,
                                 na.action = "na.fail")
summary(model_constant10)

model_constant11 = trends_populations %>% 
  dplyr::mutate(mindist_edge = pmin(reldist_trailing, reldist_leading),
                mindist_edge_scaled = scale(mindist_edge)) %>% 
                  glmmTMB(constant ~ -1 + Group +
                                   (mindist_edge_scaled * lat_abs_scaled) +
...
summary(model_constant11)


model_constant12 = trends_populations %>% 
  dplyr::mutate(mindist_edge = pmin(reldist_trailing, reldist_leading),
                mindist_edge_scaled = scale(mindist_edge)) %>% 
                  glmmTMB(constant ~ -1 + mindist_edge_scaled  +
                                   (Group * lat_abs_scaled) +
...
summary(model_constant12)


model_constant13= trends_populations %>% 
  dplyr::mutate(mindist_edge = pmin(reldist_trailing, reldist_leading),
                mindist_edge_scaled = scale(mindist_edge)) %>% 
                  glmmTMB(constant ~ -1 + 
                                   (mindist_edge_scaled * lat_abs_scaled) +
...
summary(model_constant13)

model_constant14= trends_populations %>% 
  dplyr::mutate(mindist_edge = pmin(reldist_trailing, reldist_leading),
                mindist_edge_scaled = scale(mindist_edge)) %>% 
                  glmmTMB(constant ~ -1 + 
                                   (mindist_edge_scaled * lat_abs_scaled) +
...
summary(model_constant14)

model_constant15= trends_populations %>% 
  dplyr::mutate(mindist_edge = pmin(reldist_trailing, reldist_leading),
                mindist_edge_scaled = scale(mindist_edge)) %>% 
                  glmmTMB(constant ~ -1 + 
                                   (Group * lat_abs_scaled) +
...
summary(model_constant15)

model_constant16= trends_populations %>% 
  dplyr::mutate(mindist_edge = pmin(reldist_trailing, reldist_leading),
                mindist_edge_scaled = scale(mindist_edge)) %>% 
                  glmmTMB(constant ~ -1 + mindist_edge_scaled * Group +
                                   (Group * lat_abs_scaled) +
...
summary(model_constant16)

AIC_constant = AICc(model_constant1, model_constant2, model_constant3, model_constant4, model_constant5, model_constant6, model_constant7, model_constant8, model_constant9, model_constant10,model_constant11, model_constant12, model_constant13 ,model_constant14, model_constant15, model_constant16)
AIC_constant %>% 
  sort(AICc)
...
(emm_increase = emmeans::emmeans(model_constant10, specs = pairwise ~ Group , type = "response", adjust = "none"))[2]

performance(model_constant10) # low R2

mod_data_constant = get_model_data(model_constant10, terms = c("mindist_edge_scaled", "Group"), type = "pred") %>% 
  as.data.frame()
```


``` {r}
plot_constant = plot(ggeffects::ggpredict(model_constant10, terms = c("mindist_edge_scaled", "Group"), typical = "weighted_mean", interval = "confidence"))+
  scale_fill_manual(values = cols1) +
  scale_color_manual(values = cols1) + theme_classic() +
  xlab("Relative distance to closest edge") +
  ylab("Probability of being constant") 
...
cols3 = c("-1" = "orange",
          "0"= "purple",
          "1" = "black")
plot_constanta = plot(ggeffects::ggpredict(model_constant10, terms = c("lat_abs_scaled"),
                                         type = "fixed", typical = "mode", interval = "prediction"))+
  theme_classic() +
  xlab("Minimum distance to any edge") +
  ylab("Probability of being constant")  +
  scale_fill_manual(values = cols3) +
  scale_color_manual(values = cols3, labels = c("28.47º", "41.16º", "53.85º"))
print(plot_constanta)
# getting the slope of relative distance to trailing edge on the log-odds of probability of increasing
fixed_slope_constant <- fixef(model_constant10)$cond["mindist_edge_scaled"]


# by selecting only the random effect of species (nested within group), converting values from the log-odds to probability 
ranef_mod_constant= ranef(model_constant10) %>% 
  as.data.frame() %>% 
  dplyr::filter(grpvar != "last_year") %>% 
  tidyr::separate(grp, c("Species", "Group"),sep=":",remove=F) %>% 
  dplyr::mutate(prob_intercept_constant = ifelse(term=="(Intercept)", plogis(.$condval), "NA" ), # estimated probability that specific species being constant at that average distance to the edge (since we scaled relative distance to leading edge)
                logit_lower_constant = ifelse(term=="(Intercept)", plogis(condval - 1.96 * condsd), "NA"), # lower bound of confidence interval
                logit_upper_constant = ifelse(term=="(Intercept)",plogis(condval + 1.96 * condsd), "NA"),  # upper bound of confidence interval
                group_slopes_constant = ifelse(term=="mindist_edge_scaled", .$condval + fixed_slope_constant, "NA")) # species-specific slope


plot_species_slope_constant = ggplot(ranef_mod_constant, aes(x = as.factor(Group), y = as.numeric(group_slopes_constant), fill = factor(Group))) + 
  stat_halfeye(adjust = 0.9,
               justification = -.1,
               .width = 0.01,
               point_colour = NA) +
  geom_boxplot(alpha = 0.6,
               width = .1) +
  scale_fill_manual(values = cols1) + theme_classic(base_size = 17) + geom_hline(yintercept = 0, color = "black", size = .7, linetype = 2) +
  ylab("Slope of probability of being constant x Minimum distance to nearest edge") + 
  xlab("") +  theme(legend.position="none",
                    axis.text.x=element_text(size=15)) +
  coord_flip()

print(plot_species_slope_constant)
ggsave(plot = last_plot(),"slope_speciesgroup_decreasing_trailing.jpg")
```