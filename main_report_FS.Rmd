...
```{r}
### DECREASE DATA VIZ AND ESTIMATES ####
...
decrease_slopes = ranef_mod_decrease %>% 
  dplyr::select(Species, Group, group_slopes_decrease) %>% 
  dplyr::filter(!is.na(group_slopes_decrease))
...
plot_increase2a = plot(ggeffects::ggpredict(increase_model, terms = c("lat_abs_scaled"),
                                         type = "fixed", typical = "weighted_mean", interval = "prediction"))+
  theme_classic() +
  xlab("Scaled absolute latitude") +
  ylab("Probability of increasing")  
...
increase_slopes = ranef_mod_increase %>% 
  dplyr::select(Species, Group, group_slopes_increase) %>% 
  dplyr::filter(!is.na(group_slopes_increase))
